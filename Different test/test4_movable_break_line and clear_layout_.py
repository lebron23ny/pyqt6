...
import sys
from BreakLine_ import BreakLineH
...
class MainWindow(QMainWindow):
    ...
    def InitializeComponent(self):
        ...
        #self.layout.addWidget(self.break_lineV)
        self.layout.addWidget(self.label22)
        #self.layout.addWidget(self.break_line4)
        ...
        self.layout_central_widget.addWidget(self.break_lineH)
        ...
        self.layout_central_widget.setStretch(5, 2)
        self.layout_central_widget.setStretch(6, 2)
        self.layout_central_widget.setStretch(7, 2)
        self.layout_central_widget.setStretch(8, 3)
    ...
    def click_btn2(self):
        index = self.layout_central_widget.indexOf(self.break_lineH)

        widget_top = self.layout_central_widget.itemAt(index - 1)
        widget_bottom = self.layout_central_widget.itemAt(index + 1)
        ...
        height_top_widget = widget_top.widget().height()
        height_bottom_widget = widget_bottom.widget().height()
        h_check = self.label5.height()
        width_top_widget = widget_top.widget().width()
        width_bottom_widget = widget_bottom.widget().width()
        w_check = self.label5.width()

        widget_bottom.widget().setFixedHeight(height_bottom_widget + 10)
        widget_top.widget().setFixedHeight(height_top_widget - 10)

        widget_bottom.widget().resize(width_bottom_widget, height_bottom_widget + 10)
        widget_top.widget().resize(width_top_widget, height_top_widget - 10)

        #self.frameGeometry().setHeight(1400)
        self.resize(QSize(width_window, height_window))



        geom = widget_top.geometry()
        widget_bottom = self.layout_central_widget.itemAt(index + 1)
    ...
    def click_btn3(self):
        size_Policy_H = self.label5.sizePolicy().horizontalPolicy()
        size_Policy_V = self.label5.sizePolicy().verticalPolicy()
        print(size_Policy_H)
        print(size_Policy_V)


        layout = self.button3.layout()


        count_widget = self.layout_central_widget.count()
        print(count_widget)
        stretch_list = []
        height_widget_list = []
